...
* Spring Boot 3.1 or later for this line, 4.0 or later for the 2.0.x one
...
1.0.x is written for Spring Boot 3, 2.0.x for Spring Boot 4. They are not one jar built twice, so
...
by an on condition and the parts of a date, so 3.1 is where this line starts. Everything below,
and every test behind it, is the 1.0.x line on Spring Boot 3.
...
<dependency>
    ...
    <version>1.0.0</version>  <!-- Spring Boot 3; on Spring Boot 4 take the 2.0.x line -->
</dependency>
```

While `1.0.0` is still being cut, `1.0.0-SNAPSHOT` goes to the snapshot repository of the Central
...
<dependency>
    ...
    <version>4.0.4</version>  <!-- 4.x here, Jakarta Persistence 3.1; the 2.0.x line takes 5.0 -->
</dependency>
```
...
* **SQLite** is missing enough of SQL that three of the tests have nothing to run against: no
...
* **SQLite on EclipseLink is not supported.** EclipseLink ships no SQLite platform, so it falls
  back to a generic one and writes the JDBC escapes `{fn ...}` and `{d ...}` that the SQLite driver
  does not parse. It starts and runs, and then some thirty tests have nothing to stand on. Use
  Hibernate, or the Criteria API alone, against SQLite.
* **Oracle 23 and later, on Hibernate: joining a derived table.** From Oracle 23 a group by may
  name a select item by its alias, and Hibernate answers that by writing the alias rather than the
  expression — but the alias it writes is the one it numbered internally, `c0`, while the select
  item carries the name the query gave it. `group by c0` names nothing, and Oracle says so.
...
These are the versions each one was run against, on Spring Boot 3.5 and Hibernate 6.6:

| Database | Version | Driver |
| --- | --- | --- |
| H2 | 2.3.232 | 2.3.232, in memory |
| SQLite | 3.49.1 | sqlite-jdbc 3.49.1.0 |
| MySQL | 9.6.0 | mysql-connector-j 9.7.0 |
| PostgreSQL | 16.13 | postgresql 42.7.11 |
| SQL Server | 2022, 16.00.4265 | mssql-jdbc 12.10.2 |
| Oracle | AI Database 26ai Free, 23.26.3.0.0 | ojdbc11 23.7.0 |

Every driver version is the one Spring Boot 3.5 manages, so a project on that Boot gets them by
adding the dependency and nothing else. EclipseLink is 4.0.4 here, the one Jakarta Persistence 3.1
asks for.
...
lists it: one concatenation asked for as a string on SQL Server, the three SQLite has no SQL for,
the derived table Hibernate writes a wrong group by for on Oracle 23 and later, and the paginated
Tuple EclipseLink writes without aliases on Oracle. EclipseLink against SQLite is not supported at
all, since no platform ships for it.
